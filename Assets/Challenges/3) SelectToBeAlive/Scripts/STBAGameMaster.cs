...
namespace Challenge.SelectToBeAlive
{
    public class STBAGameMaster : SystemMono
    {
        ...
        private void Awake()
        {
            Debug.Log("Result of DialogSystem Init :" + dialogSystem.Init(new DialogParser(), dialogDisplayer));
            // 여긴 커플링 해도 상관없다만... (하나밖에 안쓰고 프로젝트 활용단이라서)
            // 게임 마스터도 결국 나중엔 ChallengeKit으로 올라 갈 것이고, 그러면 나중에 뭔가 보이겠지만 지금은 그냥 쓰자.
            Debug.Log("Result of ScenarioSystem Init :" +scenarioSystem.Init(new ScenarioParser()));
